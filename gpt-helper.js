...
async function processEmail(message, message_field_search, text, imageprompt, search) {
    ...
    async function tryProcessEmail() {
        ...
        if (error.response && error.response.status === 429) {
            console.log('Received a 429 status code, retrying in 3 seconds...');
        } else {
            console.log('An error occurred, retrying in 3 seconds...');
        }
...
async function extractTextGPT(text) {
    ...
    async function tryextractTextGPT() {
        ...
        try {
            ...
            console.log(error);
            if (error.response && error.response.status === 429) {
                console.log('Received a 429 status code, retrying in 5 seconds...');
            } else {
